...
Page({
  ...
  goTaskPage(e) {
    ...
    const query = [
      ...
      `activity_id=${encodeURIComponent(info.activity_id || camp_id || '')}`,
      `project_id=${encodeURIComponent(info.project_id || '')}`,
      `group_id=${encodeURIComponent(info.group_id || '')}`,
      `member_id=${encodeURIComponent(info.member_id || '')}`,
