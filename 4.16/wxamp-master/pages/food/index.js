...
Page({
  ...
  buildCampQuery() {
    const { isCampMode, campId, activityId, projectId, groupId, memberId, campName } = this.data
    if (!isCampMode || !campId) return ''
    let query = '&camp_id=' + encodeURIComponent(campId)
    query += '&activity_id=' + encodeURIComponent(activityId || campId)
    ...
    query += '&camp_name=' + encodeURIComponent(campName || '')
    query += '&fromCamp=1'
    return query
  },

onLoad(options) {
  const campId = options.camp_id || options.activity_id || options.campId || ''
  const activityId = options.activity_id || options.camp_id || campId || ''
