...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      camp_id: options.camp_id || options.activity_id || '',
      activity_id: options.activity_id || options.camp_id || '',
  ...
  submit() {
    ...
    if (this.data.fromCamp) {
      query.camp_id = this.data.camp_id
      query.activity_id = this.data.activity_id || this.data.camp_id
      query.project_id = this.data.project_id || ''
      query.group_id = this.data.group_id || ''
      query.member_id = this.data.member_id || ''
      query.from_camp = 1
      query.camp_context_json = JSON.stringify({
