...
Page({
  ...
  handleCourseFollow() {
    ...
    if (activeCamp && activeCamp.camp_id) {
      ...
      query.push(`activity_id=${encodeURIComponent(activeCamp.activity_id || activeCamp.camp_id)}`)
    }
    if (activeCamp && activeCamp.project_id) {
      query.push(`project_id=${encodeURIComponent(activeCamp.project_id)}`)
    }
    if (activeCamp && activeCamp.group_id) {
      query.push(`group_id=${encodeURIComponent(activeCamp.group_id)}`)
    }
    if (activeCamp && activeCamp.member_id) {
      query.push(`member_id=${encodeURIComponent(activeCamp.member_id)}`)
    }
