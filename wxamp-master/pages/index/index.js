...
Page({
  ...
  goCampCenter() {
    const latestCamp = this.data.campSummary.latestCamp
  
    if (latestCamp && latestCamp.camp_id) {
      wx.navigateTo({
        url: `/pages/user/campDetail/index?camp_id=${latestCamp.camp_id}`
      })
      return
    }
  ...
  getWeight() {
    ...
    api.movewell.getBalance({
      ...
      if(res.code == 200) {
        ...
        this.setData({
          balance: res.data[date]?res.data[date]:{}
        })
        query
          .select('#canvas')
          .fields({
    ...
    api.movewell.mycycle().then(res=>{
      if(res.code == 200) {
        if(res.data.length>0){
          ...
          let channel = this.data.userInfo.channel[0]
          let weight = cycle.weight[0].weight*1
          let prose =  parseFloat((weight - channel.weight*1).toFixed(2))
  ...
  getCampSummary() {
    api.user.myCamps({}).then(res => {
      console.log('index mycamps res = ', res)
  
      if (Number(res.code) === 200) {
        const data = res.data || {}
        const futureCamps = data.future_camps || []
        const currentCamps = data.current_camps || []
        const historyCamps = data.history_camps || []
      ...
      console.log('index getCampSummary error = ', err)
      if (DEV_USE_MOCK) {
        this.fillMockCampSummary()
      }
    })
