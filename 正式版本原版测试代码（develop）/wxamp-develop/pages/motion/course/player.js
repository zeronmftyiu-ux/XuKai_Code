...
Page({
  ...
  onLoad(options = {}) {
    ...
    const courseId = Number(options.course_id || options.id || 0)
  ...
  handleVideoLoaded(e) {
    ...
    this.setData({
      ...
      statusText: this.data.courseInfo.videoUrl ? '可开始跟练' : '未配置视频'
    })
  ...
  handleVideoPlay() {
    if (!this.data.courseInfo.videoUrl) {
      wx.showToast({
        title: '未配置视频地址',
        icon: 'none'
      })
  ...
  buildCourseCheckinPayload(now) {
    const { courseInfo } = this.data
    const effectiveSeconds = Math.floor(this.data.watchedMaxTime || 0)
    const durationSeconds = Number(this.data.durationSecond || courseInfo.durationSeconds || courseInfo.duration_seconds || 0)
    const completionRate = durationSeconds > 0
      ? Number((effectiveSeconds / durationSeconds).toFixed(4))
      : 0
    const confirmDeadlineAt = this.data.endedAt
      ? this.data.endedAt + CONFIRM_EXPIRE_SECOND * 1000
      : 0

    return {
      course_id: Number(courseInfo.courseId || courseInfo.course_id || 0),
      course_name_snapshot: courseInfo.courseName || courseInfo.course_name || courseInfo.title || '',
      play_started_at: this.formatDateTime(this.data.startedAt),
      play_ended_at: this.formatDateTime(this.data.endedAt || now),
  ...
  async confirmFinish() {
    ...
    const record = {
      id: `course_${courseInfo.courseId || courseInfo.course_id || courseInfo.id}_${now}`,
      date: today,
      camp_id,
      activity_id,
      project_id,
      ...
      member_id,
      camp_name,
      courseKey: courseInfo.courseId || courseInfo.course_id || courseInfo.id,
      course_id: Number(courseInfo.courseId || courseInfo.course_id || courseInfo.id || 0),
      course_code: courseInfo.courseCode || courseInfo.course_code || '',
      title: courseInfo.courseName || courseInfo.course_name || courseInfo.title || '',
      source: '课程跟练',
      calorie: Number(courseInfo.calorie || 0),
      icon: 'https://oss.mcloud.moveclub.cn/2025/mv/sports.png',
      type: 'course_follow',
      duration_text: `${Math.floor((courseInfo.durationSeconds || courseInfo.duration_seconds || 0) / 60)}分钟`,
    ...
    if (fromCamp && activity_id) {
      const submitPayload = this.buildCourseCheckinPayload(now)
      const submitRes = await courseService.submitCampCourseCheckin(submitPayload)
