...
Page({
  ...
  toggleDate(e){
    if(moment().format('YYYY-MM-DD') < e.currentTarget.dataset.date){
      wx.showToast({
        title: '不可打卡未来日期哦～',
        icon: 'none'
      });
      ...
      if (r.code == 200) {
        ...
        if(r.data.length == 0){
          ...
          this.setData({
            ...
            now_percent: ((Number(r.data[0].amount)/Number(this.data.total_water))*100).toFixed(2)>=100?100:((Number(r.data[0].amount)/Number(this.data.total_water))*100).toFixed(2),
            remain_water: (Number(this.data.total_water) - Number(r.data[0].amount))<0?0.00:(Number(this.data.total_water) - Number(r.data[0].amount)),
          })
        }
      }
    })
  ...
  getstandard() {
    api.movewell.getStandard({signtype: 2}).then(r => { 
      if (r.code == 200) {
        this.setData({
          total_water: r.data.value
        })
        this.getData()
      }
    })
