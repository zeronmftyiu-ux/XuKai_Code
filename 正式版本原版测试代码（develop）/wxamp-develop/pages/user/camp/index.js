...
Page({
  ...
  async getCampList() {
    ...
    try {
      ...
      if (Number(res.code) === 200) {
        ...
        this.setData({
          futureCamps: (summary.futureCamps || []).map(item => ({
            ...item,
            camp_name: item.activityName || item.camp_name || `训练营${item.activityId || ''}`,
          ...
          currentCamps: (summary.currentCamps || []).map(item => ({
            ...item,
            camp_name: item.activityName || item.camp_name || `训练营${item.activityId || ''}`,
            company_name: item.companyName || item.company_name || '--'
          })),
      ...
      console.log('getCampList error = ', err)
      this.setData({
        futureCamps: [],
        currentCamps: [],
        historyCamps: [],
  ...
  goDetail(e) {
    ...
    const activityId = item.activityId || item.activity_id || item.camp_id || ''
    ...
    wx.navigateTo({
      url: `/pages/user/campDetail/index?activity_id=${activityId}&camp_id=${activityId}`
    })
  }
})