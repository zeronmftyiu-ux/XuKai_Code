    ...
    api.movewell.setCycle(query).then(res=>{
      if(res.code == 200) {
        ...
        if(userInfo.guide.question == 0) {
          wx.navigateTo({
            url: `/pages/lead/sixth/index?type=${type}&sign=${this.data.sign}`
          })
        } else {
          // wx.showToast({ title: '请勿重复填写问卷', icon: 'success' });
          wx.navigateTo({
            url: `/pages/lead/seven/index?sign=${this.data.sign}`
          })
          console.log('用户已重新填写计划')
        }
      }
