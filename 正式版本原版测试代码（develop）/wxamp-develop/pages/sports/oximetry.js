...
Page({
  ...
  getData() {
    ...
    data.healthy_type = 'com.huawei.instantaneous.spo2'
      ...
      if (res.code == 200) {
        ...
        for (var key in res.data) {
          if (!Array.isArray(res.data[key])) {
            t++
            xData.push(key)
            if (Number(res.data[key].avgvalue) < 70) {
              var items = {
                value: Number(res.data[key].avgvalue),
                itemStyle: {
                  color: '#CF363C'
                }
              }
            } else if (Number(res.data[key].avgvalue) >= 70 && Number(res.data[key].avgvalue) < 90) {
              var items = {
                value: Number(res.data[key].avgvalue),
                itemStyle: {
                  color: '#F58A32'
